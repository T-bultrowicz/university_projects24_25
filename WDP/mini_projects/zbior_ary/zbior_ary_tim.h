...
typedef struct ciag {
    int reszta; //reszta z dzielenia wyrazów ciągu przez q
    int first; //pierwszy element
    int last; //ostatni element
} ciag;

typedef struct zbior_ary {
    int rozmiar; //liczba ciągów z których składa się zbiór
    ciag* ciagi; //tablica ciągów w zbiorze
} zbior_ary;
